...
inspect(head(ProductTypes))
str(ProductTypes)
summary(transactions)
str(transactions)

#Split the data set
transactions.company <- transactions[size(transactions) > 7]
ProductTypes.company<- aggregate(transactions.company, by = "category")

transactions.individual <- transactions[size(transactions) < 8]
ProductTypes.individual<- aggregate(transactions.individual, by = "category")


#save as curiosity - attempt to split dataset
#transactions@data@p <- c(ifelse(size(transactions)<9,"individual","company"))
#data.individual <- transactions[transactions == "individual",]
...
#corporate[c(index)]<- tr
    #index <- index+1
    #tr[length(tr)+1] <- "corporate"
transaction.list[1]
transaction.list[1][length(transaction.list[1])+1] <- "corporate"
buyer
length(tr)
tr  
...
#Printers:"Brother Printer","Canon Office Printer","DYMO Label Manker","Epson Printer","HP Wireless Printer"
#Tablets:"Fire HD Tablet","iPad","iPad Pro","Kindle","Samsung Galaxy Tablet"
...
str(transactions.individual)
inspect(transactions.individual)
onlyseven <- transactions[size(transactions) == 7]
onlyseven
inspect(tail(onlyseven))
```
